...
```
##Create function for Candidate Sentiment Analysis
```{r}
sentiment_analysis <- function(sentiment){
  print(mean(sentiment))
  
  qplot(joebiden_sentiment$sentiment, geom="histogram", xlab = "Sentiment Score", ylab = "Number of Tweets", binwidth=0.1, main="Democratic Debate Tweet Sentiment Historgram")
  
}
```

##Joe Biden Tweet Sentiment Analysis

```{r}
sentiment <- sentiment(df$text[df$joebiden])
sentiment_analysis(sentiment$sentiment)
```

##Elizabeth Warren Tweet Sentiment Analysis

```{r}
sentiment <- sentiment(df$text[df$elizabethwarren])
sentiment_analysis(sentiment$sentiment)
```

##Bernie Sanders Tweet Sentiment Analysis

```{r}
sentiment <- sentiment(df$text[df$berniesanders])
sentiment_analysis(sentiment$sentiment)
```
